...
const EVENTS = {
  'new-earn-page': {
    ...
    description: <span>
      Users can now auto deposit esMYC into the vesting contract and claim Market Making Rewards via selling a portion of their MLP position
    </span>,
    continueLink: '/earn'
  },
}
...
export default function EventModal ({ isModalVisible, setEventModalVisible, twitterText, eventKey }) {
  ...
  return (
    <Styled.EventModal isVisible={isModalVisible} setIsVisible={setEventModalVisible}>
      ...
      <Styled.EventModalTitle>
        {eventText.title}
      </Styled.EventModalTitle>
      <Styled.EventModalButtonContent className="Button-content">
        {eventText.description}
        {eventText.continueLink ?
          <Link to={eventText.continueLink} onClick={() => setEventModalVisible(false)} className="App-button-option App-card-option">
            Continue
          </Link>
        :
